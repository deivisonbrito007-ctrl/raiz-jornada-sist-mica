...
export function MolduraEntrada({ frase, children }: { frase: string; children: ReactNode }) {
  return (
    <div
      className="relative isolate flex min-h-screen flex-col justify-center overflow-hidden bg-floresta md:px-6 md:py-12"
    ...
    >
      ...
      <main className="mx-auto w-full max-w-md overflow-hidden bg-background md:rounded-[2.5rem] md:shadow-organico">
        ...
        <div className="relative z-10 -mt-8 rounded-t-[2.5rem] bg-background px-6 pb-[calc(env(safe-area-inset-bottom)+3rem)] pt-8 md:px-10 md:pb-12">
          {children}
        </div>
      </main>

      <ul className="mx-auto mt-8 hidden w-full max-w-2xl flex-wrap items-center justify-center gap-x-6 gap-y-2 px-6 text-center text-xs text-floresta-foreground/80 md:flex">
        {BENEFICIOS.map((item) => (
