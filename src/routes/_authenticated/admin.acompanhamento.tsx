...
export const Route = createFileRoute("/_authenticated/admin/acompanhamento")({
  head: () => ({
    meta: [
      { title: "Acompanhamento e apoio — Raiz" },
      {
        name: "description",
        content:
          "Leia check-ins, responda pedidos de apoio e ajuste o prazo de resposta e os contatos de emergência.",
      },
      { property: "og:title", content: "Acompanhamento e apoio — Raiz" },
      {
        property: "og:description",
        content: "Check-ins, pedidos de apoio e revisões dos clientes acompanhados.",
      },
    ],
  }),
  component: AdminAcompanhamento,
});

function AdminAcompanhamento() {
  ...
  return (
    <section className="space-y-8">
      <header>
        <h1 className="font-display text-2xl text-floresta">Acompanhamento e apoio</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          O que os clientes registraram e os pedidos de apoio que aguardam retorno.
        </p>
      </header>
                ...
                <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">
                      ...
                      {s.intensidade !== null ? ` · intensidade ${s.intensidade}/10` : ""}
                    </p>
                  </div>
      ...
      <div className="rounded-3xl border border-border bg-card p-5 shadow-organico">
        <h2 className="font-display text-lg text-floresta">Check-ins recentes</h2>
        {(data?.checkins ?? []).length === 0 ? (
          <p className="mt-2 text-sm text-muted-foreground">Nenhum check-in registrado ainda.</p>
        ) : (
              ...
              >
                ...
                <span
                  className={`shrink-0 rounded-full px-3 py-1 text-xs font-medium ${
                    c.intensidade >= 8
                      ? "bg-terracota text-terracota-foreground"
